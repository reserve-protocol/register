...
const SpacingResult = () => (
  <div className="space-y-5">
    ...
    <div className="grid gap-px bg-border sm:grid-cols-2 lg:grid-cols-4">
      <RuleValue label={<>Tight text</>} value="4px" />
      <RuleValue label={<>Related content</>} value="8px" />
      <RuleValue label={<>Nested inset</>} value="16px" />
      <RuleValue label={<>Section / group</>} value="24px" />
    </div>
...
const RadiusResult = () => (
  <div className="grid gap-px bg-border sm:grid-cols-3">
    ...
    >
      <>Page sections, table regions, and modal task structure.</>
    </RadiusRole>
    <RadiusRole
      label={<>Contained object</>}
      value="8px"
      className="rounded-lg"
    >
      <>Multiline fields, menus, popovers, and amount regions.</>
    </RadiusRole>
    <RadiusRole
      label={<>Atomic control</>}
      value={<>Full</>}
      className="rounded-full"
    >
      <>Buttons, tabs, one-row fields, switches, and status.</>
    </RadiusRole>
  </div>
)
...
const ElevationResult = () => (
  <div className="grid gap-px bg-border sm:grid-cols-3">
    <LayerRole label={<>Level 0</>} value={<>Flat structure</>}>
      <>Surface contrast and seams carry ordinary hierarchy.</>
    </LayerRole>
    <LayerRole label={<>Level 1</>} value={<>Floating</>}>
      <>Menus, popovers, tooltips, and overlapping wrappers.</>
    </LayerRole>
    <LayerRole label={<>Level 2</>} value={<>Overlay</>}>
      <>Dialogs and temporary task layers use stronger separation.</>
    </LayerRole>
...
const MotionResult = () => (
  <div className="grid gap-px bg-border sm:grid-cols-3">
    <MotionRole value="120ms" label={<>Immediate</>}>
      <>Hover, press, color, opacity</>
    </MotionRole>
    <MotionRole value="180ms" label={<>Standard</>}>
      <>Disclosure, tabs, small surfaces</>
    </MotionRole>
    <MotionRole value="240ms" label={<>Spatial</>}>
      <>Dialog, drawer, meaningful entrance</>
    </MotionRole>
...
const IconographyResult = () => (
  <div className="grid gap-px bg-border sm:grid-cols-3">
    <IconRole label={<>Micro</>} value="14px">
      <Search className="size-3.5" strokeWidth={1.5} />
    </IconRole>
    <IconRole label={<>Control</>} value="16px">
      <Focus className="size-4" strokeWidth={1.5} />
    </IconRole>
    <IconRole label={<>Content</>} value="20px">
...
const AccessibilityResult = () => (
  <div className="grid gap-px bg-border sm:grid-cols-2 lg:grid-cols-3">
    <Guardrail icon={<Focus />} label={<>Visible focus</>}>
      <>Keyboard-operated primitives expose a clear focus indicator.</>
    </Guardrail>
    <Guardrail icon={<Check />} label={<>44px target</>}>
      <>Compact visuals retain an adequate touch and pointer hit area.</>
    </Guardrail>
    <Guardrail icon={<TriangleAlert />} label={<>Meaning beyond color</>}>
      <>Status combines readable text, structure, or a recognizable symbol.</>
    </Guardrail>
    <Guardrail icon={<Search />} label={<>Accessible names</>}>
      <>Visible and announced labels describe the same action.</>
    </Guardrail>
    <Guardrail icon={<Circle />} label={<>Dialog focus</>}>
      <>Focus enters, stays inside, dismisses, and returns predictably.</>
    </Guardrail>
    <Guardrail icon={<MoveRight />} label={<>Reduced motion</>}>
      <>
