...
export interface GovernanceProposalFixture {
  ...
  countdown?: { label: string; value: string; deadline?: boolean }
  outcome?: 'passed'
...
export const CURRENT_PROPOSAL_FIXTURES: GovernanceProposalFixture[] = [
  {
    ...
    countdown: { label: 'Voting starts in', value: '18h' },
    stages: [
      {
        key: 'proposal',
        duration: 2,
  ...
  },
  {
    ...
    countdown: { label: 'Voting ends in', value: '6h', deadline: true },
  ...
  },
  {
    ...
    countdown: { label: 'Challenge ends in', value: '11h', deadline: true },
    stages: [
  ...
  },
  {
    ...
    qualifier: 'contested',
    statuses: [{ label: 'Voting active', role: 'active', indicator: 'voting' }],
    countdown: { label: 'Voting ends in', value: '2d', deadline: true },
    stages: STANDARD_ACTIVE_STAGES,
    progressEmphasis: 'strong',
    evidence: {
      kind: 'standard',
  ...
  },
  {
    ...
    kind: 'standard',
    statuses: [{ label: 'Waiting period', role: 'waiting' }],
    outcome: 'passed',
    waitingPeriod: true,
    countdown: { label: 'Execution available in', value: '8h' },
    stages: [
      { key: 'proposal', duration: 2, status: 'completed' },
      { key: 'voting', duration: 5, status: 'completed' },
      {
  ...
  },
  {
    ...
    title: 'September 2026 Rebalance',
    kind: 'standard',
    statuses: [{ label: 'Ready to execute', role: 'actionable' }],
    outcome: 'passed',
    stages: [
      { key: 'proposal', duration: 2, status: 'completed' },
      { key: 'voting', duration: 5, status: 'completed' },
...
export const HISTORICAL_PROPOSAL_FIXTURES: GovernanceProposalFixture[] = [
  {
    ...
    title: 'July 2026 Governance Parameter Update',
    kind: 'standard',
    statuses: [{ label: 'Proposal expired', role: 'closed' }],
    stages: STANDARD_DECIDED_STAGES,
