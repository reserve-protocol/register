          ...
          <div data-testid="staged-orders-list">
            {state === 'Quote searching' ? (
              ...
              <EmptyOrders
                ...
                detail={<>Some quotes couldn't be fetched.</>}
                tone="danger"
              />
            ) : state === 'No swaps needed' ? (
              <EmptyOrders
                title={<>No swaps needed</>}
                detail={<>You can proceed directly.</>}
              />
            ) : (
...
const QuoteSearchRows = () => {
  return (
    <div aria-label={`Fetching swap quotes`}>
      {[0, 1, 2].map((item) => (
        <div
          key={item}
...
const getOrdersDescription = (
  ...
  if (state === 'Quote paused') return <>Fetching paused.</>
  if (state === 'Quote unavailable') {
    return <>Some quotes couldn't be fetched.</>
  }
  if (state === 'No swaps needed') {
    return <>All required collateral is already available.</>
  }
  if (state === 'Mint complete' || state === 'Redeem complete') {
    return operation === 'mint' ? (
      <>Collateral swaps for this mint.</>
    ) : (
      <>Collateral swaps for this redeem.</>
    )
  }
  if (
    ...
    return <>Swaps settle via CoW Protocol solvers.</>
  }
  return operation === 'mint' ? (
    <>Your input will be split across these basket assets.</>
  ) : (
