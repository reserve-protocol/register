...
export function WeightError({
  ...
  return (
    ...
    >
      {limit
        ? 'Lab: enter a non-negative number with up to 18 decimal places, or leave this limit empty.'
        : 'Lab: enter a non-negative number with up to 18 decimal places.'}
    </FieldMessage>
  )
}
      ...
      {draft.every(validDecimal) &&
        !draft.some((value) => /[1-9]/.test(value)) && (
          <FieldMessage role="status">
            Lab: at least one token must have units greater than zero.
          </FieldMessage>
        )}
      {!enabled && (
        <FieldMessage role="status">
          Lab: reconnect the launcher on the correct network with available data
          to save. Your draft is retained.
        </FieldMessage>
      )}
    </>
  )
