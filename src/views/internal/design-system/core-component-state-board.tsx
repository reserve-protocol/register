...
const ActionBoard = () => (
  <Board title="Action variants and states" detail="Default · 44px">
    <div className="grid grid-cols-2 gap-px bg-border xl:grid-cols-4">
      <ActionCell label="Primary">
        <Button>Continue</Button>
      </ActionCell>
      <ActionCell label="Secondary">
        <Button tone="secondary">Save draft</Button>
      </ActionCell>
      <ActionCell label="Quiet">
        <Button tone="quiet">Cancel</Button>
      </ActionCell>
      <ActionCell label="Destructive">
        <Button tone="destructive">Delete</Button>
      </ActionCell>
      <ActionCell label="Primary + loading">
        <Button loading>Saving</Button>
      </ActionCell>
      <ActionCell label="Destructive + loading">
        <Button tone="destructive" loading>
          Deleting
        </Button>
      </ActionCell>
      <ActionCell label="Primary + disabled">
        <Button disabled>Continue</Button>
      </ActionCell>
      <ActionCell label="Secondary + disabled">
...
const SelectionBoard = () => (
  <Board
    title="Selection"
    detail="Important combinations · not every permutation"
  >
    <StateGrid
      labels={[
        'Default',
    ...
    >
      ...
      <SelectionCell label="Checkbox">
        <CheckboxMark checked focused />
      </SelectionCell>
      <SelectionCell label="Checkbox">
        <CheckboxMark checked disabled />
      </SelectionCell>

      <SelectionCell label="Radio">
        <RadioMark />
      </SelectionCell>
      <SelectionCell label="Radio">
        <RadioMark checked />
      ...
      <SelectionCell label="Radio">
        <RadioMark checked focused />
      </SelectionCell>
      <SelectionCell label="Radio">
        <RadioMark checked disabled />
      </SelectionCell>

      <SelectionCell label="Switch">
        <SwitchMark />
      </SelectionCell>
      <SelectionCell label="Switch">
        <SwitchMark checked />
  ...
  <Board title="Support" detail="Value, loading, placeholder">
    <div className="grid grid-cols-2 gap-px bg-border">
      ...
      <SupportCell label="Disclosure">
        <button
          type="button"
          className="flex h-11 w-full max-w-56 items-center justify-between border-y border-border text-sm font-light"
        >
...
const StateGrid = ({
  ...
  <div className="grid grid-cols-4 gap-px bg-border">
    {labels.map((label) => (
      <div
        key={label}
        className="bg-muted px-3 py-2 text-[10px] font-medium text-muted-foreground"
      >
        {label}
      </div>
    ))}
...
const CheckboxMark = ({
  ...
  <Checkbox
    ...
    disabled={disabled}
    className={
      focused ? 'ring-2 ring-ring ring-offset-2 ring-offset-card' : undefined
    }
  />
)
...
const FieldCell = ({
  ...
  <div
    className={`${span === 'full' ? 'col-span-4' : ''} flex min-h-24 items-center justify-center bg-card p-3`}
  >
    {children}
  </div>
)
...
const Field = ({
  ...
  <div className="w-full max-w-44">
    <input
      readOnly
      disabled={disabled}
      value={disabled ? 'Unavailable' : 'Proposal title'}
      className={cn(
    ...
    />
    {error && (
      <p className="mt-1 text-[10px] text-destructive">Required field</p>
    )}
  </div>
)
...
const Textarea = () => (
  <div className="grid w-full grid-cols-[7rem_1fr] items-center gap-3">
    ...
    <textarea
      readOnly
      value="Describe the proposal and its expected outcome."
      className="min-h-20 w-full resize-none rounded-lg border border-input bg-card px-4 py-3 text-base font-light outline-none"
    />
  </div>
)
...
const CandidateSlider = () => (
  <div
    className="relative h-5 w-full max-w-56"
    role="img"
    aria-label="Slider at 62 percent"
  >
    <div
      className={cn(
        'absolute inset-x-0 top-1/2 h-2 -translate-y-1/2 overflow-hidden rounded-full',
...
const Feedback = ({
  ...
  return (
    <div className="flex min-h-24 gap-3 bg-card p-4">
      <span className={`${toneClass} mt-0.5 [&>svg]:h-4 [&>svg]:w-4`}>
        {icon}
      </span>
      <div>
        <p className="text-sm font-medium">{title}</p>
        <p className="mt-1 text-xs font-light leading-5 text-muted-foreground">
          Short supporting message belongs here.
        </p>
      </div>
    </div>
  )
