...
export const ManualIssuanceAction = ({
  ...
  const message =
    failed && stage !== 'transaction'
      ? failed === 1
        ? t`Approval failed: ${failedTokens}`
        : t`Approvals failed: ${failedTokens}`
    ...
    >
        ...
        >
          ...
          {failed > 0 && (
            <HelpTooltip
              ...
              accessibleLabel={t`One or more approvals failed`}
              content={
                <Trans>
          ...
          {s.transaction === 'error' && s.error && (
            <HelpTooltip
              className="focus-visible:ring-offset-[var(--inline-message-surface)]"
              accessibleLabel={
                signingDeclined ? t`Signing declined` : t`Transaction failed`
              }
              content={s.error}
            />
          )}
      ...
      >
        {walletAction && s.gate === 'disconnected' ? (
          ...
          <Trans>Please sign in wallet...</Trans>
        ) : !approvalAction && s.transaction === 'confirming' ? (
          <Trans>Confirming transaction...</Trans>
        ) : approvalAction && failed ? (
          <Plural
            value={failed}
            one="Retry # approval"
            other="Retry # approvals"
          />
        ) : approvalAction ? (
          pending.length && s.gate !== 'loading' ? (
            t`Approve All (${pending.length})`
          ) : (
            t`Approve All`
          )
        ) : isMint ? (
          t`Mint ${s.amount || '0'} CMC20`
        ) : (
          t`Redeem ${s.amount || '0'} CMC20`
        )}
      </Button>
    </div>
  )
