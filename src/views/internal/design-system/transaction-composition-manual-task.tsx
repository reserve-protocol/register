...
export const ManualIssuanceTask = ({
  ...
  const [showZapperPreview, setShowZapperPreview] = useState(false)
        ...
        >
          ...
          >
            <p className={cn(v1Typography.supporting, roles.text.supporting)}>
              {isMint ? (
                <Trans>Want to buy with a single token?</Trans>
              ) : (
                <Trans>Having issues redeeming?</Trans>
              )}
            </p>
            <InlineAction
              ...
              disabled={busy}
              aria-describedby="manual-issuance-preview-note"
              onClick={() => setShowZapperPreview(true)}
            >
              <Trans>Switch to Zapper</Trans>
            </InlineAction>
            {showZapperPreview && !busy && (
              <p
                data-testid="manual-zapper-preview"
                role="status"
                className={cn(
                  'w-full',
              ...
              >
                <Trans>
                  Preview: Switch to Zapper opens the buy/sell flow for this
                  DTF. No swap is executed and your manual amount is unchanged.
                </Trans>
              </p>
            )}
          </div>
