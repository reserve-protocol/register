        ...
        >
          ...
          {record.countdown && (
            ...
            >
              ...
              <span>{t(record.countdown.label)}</span>{' '}
...
function ProposalHelp({ record }: { record: GovernanceProposalFixture }) {
  ...
  return (
    ...
    >
      <HelpTooltip
        accessibleLabel={
          record.waitingPeriod ? t`Waiting period` : t`Proposal timeline`
        }
        content={
          <div className="space-y-2">
            ...
            <p>
              <Trans>
                The strip represents lifecycle timing, not voting support.
              </Trans>
            </p>
            <p>
              {record.kind === 'optimistic' ? (
                <Trans>Challenge period</Trans>
              ) : (
                <Trans>Voting delay → Voting → Execution delay</Trans>
              )}
            </p>
          </div>
        }
