...
const ConfirmEligibilityModal = () => {
  ...
  const canConfirm = acceptedTerms && confirmedJurisdiction
    ...
    <Dialog open>
      ...
      >
        ...
        <div className="rounded-3xl border border-border bg-background">
          <div className="flex items-center gap-4 p-4 sm:p-5">
            <Checkbox
              ...
              aria-labelledby="eligibility-terms-label"
              checked={acceptedTerms}
              onCheckedChange={(checked) => setAcceptedTerms(checked === true)}
              className="h-8 w-8 rounded-full"
            />
            <p
              id="eligibility-terms-label"
          ...
          <div className="flex items-center gap-4 p-4 sm:p-5">
            <Checkbox
              ...
              aria-labelledby="eligibility-jurisdiction-label"
              checked={confirmedJurisdiction}
              onCheckedChange={(checked) =>
                setConfirmedJurisdiction(checked === true)
              }
              className="h-8 w-8 rounded-full"
            />
            <p
              id="eligibility-jurisdiction-label"
              className="text-base leading-snug text-foreground"
            >
              <Trans>
        ...
        <p className="mt-2 text-sm  text-legend px-4 pb-4">
          <Trans>
            Your privacy is protected. This confirmation is stored only in this
            browser - never personal financial information.{' '}
            <a
