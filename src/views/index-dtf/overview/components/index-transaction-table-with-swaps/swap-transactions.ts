...
// The subgraph only indexes DTF ERC20 transfers, so a swap is inferred from
// the PoolManager being the counterparty. Liquidity management (a v4
...
// Such round-trip counterparties are dropped for that tx.
const roundTripCounterparties = (events: PoolTransferEvent[], pm: string): Set<string> => {
...
// different counterparties renders as two honest rows, not a netted one.
export const mapPoolSwapEvents = (
  ...
  for (const event of events) {
    ...
    const counterparty = type === "Buy" ? to : from;

    if (roundTrips.has(`${event.hash}-${counterparty}`)) continue;
...
export const mergeTransactionRows = (
  ...
  return [...transactions, ...swapRows].sort((a, b) => b.timestamp - a.timestamp);
};
