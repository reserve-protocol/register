...
describe("mapPoolSwapEvents", () => {
  ...
  it("drops a liquidity round-trip: same counterparty in and out of the pool manager in one tx", () => {
    const swaps = mapPoolSwapEvents(
      {
  ...
  it("matches round-trips case-insensitively", () => {
    const swaps = mapPoolSwapEvents(
      {
        buys: [event({ id: "lp-out", to: { id: LP_VAULT.toUpperCase().replace("0X", "0x") } })],
        sells: [event({ id: "lp-in", from: { id: LP_VAULT }, to: { id: PM } })],
      },
      PM,
      CHAIN,
  ...
  it("does not treat the same counterparty in different txs as a round-trip", () => {
    const swaps = mapPoolSwapEvents(
      {
        buys: [event({ id: "a-1", hash: "0xa" })],
        sells: [event({ id: "b-1", hash: "0xb", from: { id: TRADER }, to: { id: PM } })],
      },
      PM,
      CHAIN,
    );
...
describe("mergeTransactionRows", () => {
  ...
  const swap = (timestamp: number) =>
    mapPoolSwapEvents({ buys: [event({ timestamp: String(timestamp) })], sells: [] }, PM, CHAIN)[0];
...
describe("compute24hSwapVolumeUsd", () => {
  ...
  it("sums only swaps inside the trailing 24h window", () => {
    ...
    expect(compute24hSwapVolumeUsd([inside, atCutoff, outside], 5, NOW)).toBe(10);
  });
