      ...
      <HoverCardContent className="w-[260px] rounded-3xl border-2 border-secondary p-3">
        <div className="flex flex-col gap-2 text-xs">
          ...
          {market && !market.isOpen && market.nextOpen && (
            <Row label="Next open">{formatTime(market.nextOpen)}</Row>
          )}
          {market && market.isOpen && market.nextClose && (
            <Row label="Closes">{formatTime(market.nextClose)}</Row>
          )}
          ...
          {!ondo.tradingOpen && (
            <p className="text-red-600">
              {ondo.reason?.message
                ? `Halted — ${humanizeEvent(ondo.reason.message)}`
                : 'Trading halted'}
            </p>
          )}
            ...
            <div className="flex flex-col gap-0.5 border-t pt-1">
              {ondo.upcoming.map((u, i) => (
                ...
                >
                  ...
                  <span>
                    {humanizeEvent(u.message) || u.code || 'Scheduled event'}
