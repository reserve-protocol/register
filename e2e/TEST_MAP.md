...
| Explorer | [general/explorer/render](tests/general/explorer/render.spec.ts), [general/explorer/ongoing-governance](tests/general/explorer/ongoing-governance.spec.ts) | transactions tab (default) render; governance tab proposals render; ongoing Index DTF proposal cards (loading/full/empty/partial-chain failure, deprecated DTF, mobile long title); one chain returning malformed transactions body doesn't blank the page | full (ongoing governance) | yes (ongoing governance) | filters, pagination, tokens/collaterals/revenue tabs |
