...
| Wallet modal | [flows/wallet-connect](tests/flows/wallet-connect.spec.ts) | AppKit connect modal lists the injected Test Wallet with the legal footer; connect → header chip; account view; explicit disconnect is not undone by AppKit reconnect | none | yes | WalletConnect/Safe paths are unmockable (relay stubbed) — manual |
