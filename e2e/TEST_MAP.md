...
| Auctions – rebalance detail + writes | [auctions/launch-price-guard](tests/index-dtf/auctions/launch-price-guard.spec.ts), [auctions/launch-write](tests/index-dtf/auctions/launch-write.spec.ts), [auctions/version-identity-nav](tests/index-dtf/auctions/version-identity-nav.spec.ts), [flows/auctions](tests/flows/auctions.spec.ts) | active detail from encoded `getRebalance()`; expired→completed card; price-error / single-0-price blocks launch; launcher `openAuction()`; non-launcher-in-permissionless-window `openAuctionUnrestricted()`; launch button recovers after a cached same-version A→B→A hop | none | no | legacy v2 auctions UI, bid writes; [auctions/rebalance-evidence](tests/index-dtf/auctions/rebalance-evidence.spec.ts) is screenshot evidence gated on `E2E_EVIDENCE_DIR`, not coverage |
