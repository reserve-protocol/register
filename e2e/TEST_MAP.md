...
74 specs across 5 top-level dirs: `general/` (8), `index-dtf/` (17),
`yield-dtf/` (6), `smoke/` (12), `flows/` (31). `index-dtf/` and `yield-dtf/`
hold render/lifecycle/mobile specs per route; `flows/` holds deeper
behavior/write/edge-case specs (desktop only, except `flows/wallet-connect`,
which also runs on mobile because the wallet modal is a mobile-first surface).
...
| Wallet connection | [flows/wallet-connect](tests/flows/wallet-connect.spec.ts) | Legal footer; default email/social and fund/swap/send/activity controls; iframe-only Safe option absent outside Safe; connect → account → persistent disconnect; plain address (never clipped) and short/long ENS labels with header/navigation bounds at 360/412/1280px | none | yes | Live dashboard settings, email/social authentication, payments, WalletConnect handoff and Safe iframe/relay sessions remain manual |
...
| Auctions – rebalance detail + writes | [auctions/launch-price-guard](tests/index-dtf/auctions/launch-price-guard.spec.ts), [auctions/launch-write](tests/index-dtf/auctions/launch-write.spec.ts), [auctions/version-identity-nav](tests/index-dtf/auctions/version-identity-nav.spec.ts), [flows/auctions](tests/flows/auctions.spec.ts) | active detail from encoded `getRebalance()`; expired→completed card; price-error / single-0-price blocks launch; launcher `openAuction()` (v5 five-arg, and six-arg with `maxAuctionLength` when the proxy reports 6.0.0); post-receipt gate flips from RPC (`data-ongoing`); non-launcher-in-permissionless-window `openAuctionUnrestricted()`; launch button recovers after a cached same-version A→B→A hop | none | no | legacy v2 auctions UI, bid writes; [auctions/rebalance-evidence](tests/index-dtf/auctions/rebalance-evidence.spec.ts) is screenshot evidence gated on `E2E_EVIDENCE_DIR`, not coverage; real launches live in the fork lane (`playwright.fork.config.ts`, [fork/launch-cmc20](fork/tests/launch-cmc20.fork.spec.ts)) |
