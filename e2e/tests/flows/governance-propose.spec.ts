...
// eth_sendTransaction with a unique recorded hash and the RPC mock serves its
...
test('create a DAO-settings proposal through the full submit flow', async ({
  ...
  expect(decoded.args[0].length).toBeGreaterThan(0)
  expect(decoded.args[1]).toEqual(decoded.args[0].map(() => 0n))
  expect(decoded.args[2]).toHaveLength(decoded.args[0].length)
  expect(decoded.args[3]).toContain('E2E DAO settings proposal')
})
