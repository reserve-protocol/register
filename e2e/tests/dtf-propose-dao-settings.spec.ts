/**
 * E2E tests for DAO Settings proposal form.
 * Uses stToken.governance — 3 sections: Revenue Tokens, Governance, Roles.
 */
...
test.describe('DAO Settings: Rendering', () => {
  test.beforeEach(async ({ page }) => {
    await navigateToProposalForm(page, expect, 'other')
  })
  ...
  test('shows 3 accordion sections', async ({ page }) => {
    const form = page.locator('.bg-secondary.rounded-4xl').first()
    await expect(form).toBeVisible({ timeout: 5000 })

    await expect(form.getByText('Revenue Tokens').first()).toBeVisible()
...
test.describe('DAO Settings: Revenue Tokens Section', () => {
  test('shows reward token when opened', async ({ page }) => {
    await navigateToProposalForm(page, expect, 'other')
    await openSection(page, expect, 'revenue-tokens')

    const section = page.locator('#propose-section-revenue-tokens')
    // Mock has ETH as a reward token
    await expect(section.getByText('ETH').first()).toBeVisible({
      timeout: 5000,
    })
  })
})

test.describe('DAO Settings: Governance Section', () => {
  test.beforeEach(async ({ page }) => {
    await navigateToProposalForm(page, expect, 'other')
...
test.describe('DAO Settings: Roles Section', () => {
...
test.describe('DAO Settings: Full Flow', () => {
  test('complete DAO settings proposal flow', async ({ page }) => {
    await navigateToProposalForm(page, expect, 'other')
    await waitForFormReady(page)
    ...
    await fillProposalTitle(page, 'E2E Test: Update DAO voting period')
