...
| 2. Test swarm | 7 areas delivered: home/discover, overview (multichain + mcap-framing guard), issuance (approve→mint, redeem + post-tx balance), zap buy/sell (real zapper boundary, pinned quotes), auctions (idle/historical/active via encoded getRebalance), governance (6 states, propose, queue/execute), settings + compliance (geo/VPN). Shared fixes: raw GetIndexDTF snapshots + chain-state (v4/v5 split real), api-staging host, per-DTF compliance default, list enrichment, zapper/tokens, advanceTime | done — 38 tests; full suite 38/38 + 37/38 (zap-sell flake → 2.5); 76 deterministic unmocked eth_calls in governance flows (→ 2.5) |
