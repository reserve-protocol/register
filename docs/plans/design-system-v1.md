...
| Amount relationship   | `TransactionAmountObject` / `TransactionAmountPair`, square regions, 16px internal inset, and 4px pair seam                                       | Asset selector, balance action, precision, quote terminology, and direction control follow the actual flow                                       |
...
| Action and progress   | A real Button for a required click; stable action placement; lifecycle status/progress replaces inactive action chrome while no click is possible | Approval disclosure, ordered rows, retry scope, and step count follow actual execution; the current stepper anatomy remains exploratory          |
...
Do not transfer Zapper route-choice UI, Delegation address anatomy, 448px flow
widths, attachment tone, prose emphasis, committed-mode motion, or stepper
presentation merely because they appear in the review. Stake/Unstake is the
next proof run for the relationships above; only repeated matching evidence may
...
- **Stake/Unstake proof run:** use the hardened contract with direct current-flow
  evidence to test fresh-worker reusability. This is the next selected slice;
  it must pressure-test the recipe rather than assume a third matching example.
