...
- Chart family presentation is an approved-for-now, unadopted lab baseline.
  Its evidence is frozen; engineer review remains required for adoption.
...
preserved tables approved for now and charts then under review. The later
[chart decision](../wiki/decisions.md#2026-09-16--current-chart-work-approved-for-now)
supersedes that transient status: the current chart lab is the frozen,
approved-for-now baseline, while production adoption remains separate. The authorized
[instruction-path cleanup and workflow closeout](design-system-workflow-read-paths.md)
...
The paused transaction checkpoint remains in the `transaction-action` catalog
entry rather than Current Review. Continue only through explicit human visual
...
The frozen chart baseline began with the [source-faithful chart review](design-system-charts-source-reset.md):
...
freshness rule, sampling policy or chart-type default is changed. The authorized
[candlestick addition](design-system-candlesticks.md) now prepares Overview
OHLC review; signed Factsheet bars are excluded pending confirmation of use,
and pie/allocation remains later work. Yield metric and Portfolio history studies
have completed the authorized [human-directed visual revision](design-system-next-charts/revision-plan.md):
clear chart-only review boundaries, correct readout roles, measured axis edges,
and a source-grounded Portfolio key/palette/controls treatment. The
[completion package](design-system-next-charts/completion/README.md) retains
pre-feedback evidence, including the candle tooltip and lighter Home title.
The authorized [feedback pass](design-system-next-charts/feedback-pass/plan.md)
now owns inset/axis refinement, Portfolio controls and inspection, and precise
line-end markers. The delegated candidate and independent reviews are reconciled
in its [closeout](design-system-next-charts/feedback-pass/README.md). Its former
pending visual status is superseded by the September 16 approved-for-now
decision; production adoption is not implied.
The subsequent [optical refinement](design-system-next-charts/optical-refinement/plan.md)
is authorized and delegated. Its revised scope now separates text-only visual
height from usable hit areas, replaces the oversized CSV ghost treatment,
anchors Portfolio controls optically to the top-right, fits actual displayed
axis labels and matches Overview performance weight to its price. Preserve the
already-added local mobile controls and rebalanced amount/date spacing. Shared
text-only default promotion requires consumer and interaction proof; contained
controls, ordinary Button defaults and production remain unchanged.
Its bounded [partial-history closeout](design-system-next-charts/partial-history/README.md)
now keeps normal Yield on available bounds and gives the simulated Portfolio
fixture explicit known-zero pre-holdings samples before its first positive value.
YTD, 1Y and All can inspect that known state; 7D and 1M remain post-holdings.
The superseded Price example is removed. Production still hides finite periods
older than its first positive value and does not synthesize or prepend zeros;
unknown history must never become zero. The implementation and browser evidence
are green. The September 16 decision promotes only this reviewed lab
presentation to a revisitable current baseline; engineer review remains required
before any separately authorized production adoption. Current Review is empty
and no replacement target has been selected.
...
specific row in the linked register when it changes a shared implementation contract, touches a
...
Read and update the [engineering handoff register](design-system-engineering-handoff.md)
when those triggers apply. It retains every existing review surface, owner/reason,
current boundary and evidence link. Final engineering handoff reviews the whole
register; unrelated local presentation work need not load every row.
