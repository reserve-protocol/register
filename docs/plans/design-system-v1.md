...
records why owned stake/vote-lock rows were excluded from the first slice;
Earn's optional holding column does not cover that job. The authorized
[owned-position candidate](design-system-owned-positions-slice.md) adds those two
families at `#owned-positions-review`, with source-backed identities, illustrative
balances and independent Modify boundaries. Current Review stays on Auctions.
Rewards, voting power, RSR balances, activity and Portfolio proposals remain outside
this candidate; it is not full Portfolio coverage or production adoption.
...
The [September 13 review checkpoint](design-system-review-checkpoint-2026-09-13.md)
preserves the subsequent Auctions, owned Portfolio and Discover card work for an
isolated chart-readiness audit. It is a reproducible research baseline, not visual
acceptance, production adoption or the start of chart implementation.
The earlier authorized [Auctions browse-record slice](design-system-auctions-browse-slice.md)
prepared active/history records with copied snapshot identities, explicit
illustrative outcome/activity overlays and loading/unavailable/empty/zero states.
Governance remains in its retained composition; selection, detail/actions, exact split and production
adapters are not implemented or accepted.
The [current rebalance workspace](design-system-current-rebalance-workspace.md)
is the new review candidate at `#auctions-browse-review`, above the retained
